...
fun DetailScreen(
    ...
    // Observe and collect movies state
    val movies = moviesViewModel.movies.collectAsState(initial = emptyList()).value
    // Find the specific movie by ID
    val movie = movies.find { it.id == movieId }

    movie?.let { detailedMovie ->
        Scaffold(
            topBar = {
                SimpleTopAppBar(title = detailedMovie.title) {
            ...
            Column {
                ...
                Column {
                    Text("Movie Trailer")
                    VideoPlayer(trailerURL = detailedMovie.trailer)
                }

                Divider(modifier = Modifier.padding(4.dp))

                HorizontalScrollableImageView(movie = detailedMovie)
            }
        }
